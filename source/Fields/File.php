...
class File extends AbstractField
{
    ...
    {
        wp_enqueue_media();
        \Helpers\Script::enqueue_admin_inline_script('wpkit-field-file', $this->_render_javascript());
    }
    ...
    {
        if($this->get_label()) {
            return sprintf('<label>%s</label>', $this->get_label());
        }
    ...
    {
        $class = '';
        if(!empty($this->_options['attributes']['class'])) {
            $class = $this->_options['attributes']['class'];
        }

        $desc = '';
        if(!empty($this->_options['description'])) {
            $desc = $this->_options['description'];
        }
    ...
    protected function _render_remove_button()
    {
        ...
        return sprintf( '<a id="remove_%s" data-role="file-remove" href="#" %s>%s</a>', $this->get_name(), $attr, $this->_labels['remove_button'] );
    }
        ...
        <script>
            jQuery(function () {
                ...
                jQuery('body').on('click', '[data-role="file-load"]', function (event) {
                    ...
                    $field.val('');
                    $title.empty();
                    $button.attr('class', '<?php echo $class; ?>').find('img').hide().attr('src', '');
